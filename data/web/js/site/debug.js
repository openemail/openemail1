  ...
  function draw_ui_logs() {
    ft_api_logs = FooTable.init('#ui_logs', {
      ...
      "rows": $.ajax({
        ...
        success: function (data) {
          return process_table_data(data, 'openemail_ui');
        }
      }),
      "empty": lang.empty,
      "paging": {"enabled": true,"limit": 5,"size": log_pagination_size},
  ...
  $('.add_log_lines').on('click', function (e) {
    ...
    if (ft = FooTable.get($('#' + log_table))) {
      ...
      $.get('/api/v1/get/logs/' + log_url + '/' + load_rows).then(function(data){
        if (data.length === undefined) { openemail_alert_box(lang.no_new_rows, "info"); return; }
        ...
        openemail_alert_box(data.length + lang.additional_rows, "success");
