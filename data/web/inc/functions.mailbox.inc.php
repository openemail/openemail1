      ...
      switch ($_type) {
        case 'time_limited_alias':
          ...
          if (isset($_data['username']) && filter_var($_data['username'], FILTER_VALIDATE_EMAIL)) {
            if (!hasMailboxObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $_data['username'])) {
              $_SESSION['return'][] = array(
                'type' => 'danger',
                'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
                'msg' => 'access_denied'
          ...
          $stmt->execute(array(':username' => $_SESSION['openemail_cc_username']));
          ...
          $_SESSION['return'][] = array(
            'type' => 'success',
            'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
            'msg' => array('mailbox_modified', htmlspecialchars($_SESSION['openemail_cc_username']))
          );
        break;
        case 'filter':
        ...
        case 'domain':
          ...
          if ($domain == getenv('OPENEMAIL_HOSTNAME')) {
            $_SESSION['return'][] = array(
              'type' => 'danger',
              'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
        ...
        case 'alias_domain':
          ...
          if (!hasDomainAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $target_domain)) {
            $_SESSION['return'][] = array(
              'type' => 'danger',
              'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
              'msg' => 'access_denied'
      ...
      switch ($_type) {
        ...
        case 'tls_policy':
          ...
          foreach ($usernames as $username) {
            if (!filter_var($username, FILTER_VALIDATE_EMAIL) || !hasMailboxObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $username)) {
              $_SESSION['return'][] = array(
                'type' => 'danger',
                'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
                'msg' => 'access_denied'
        ...
        case 'time_limited_alias':
          ...
          foreach ($addresses as $address) {
            ...
            if (!hasMailboxObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $goto)) {
              $_SESSION['return'][] = array(
                'type' => 'danger',
                'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
                'msg' => 'access_denied'
        ...
        case 'domain':
          ...
          foreach ($domains as $domain) {
            ...
            hasDomainAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $domain)) {
            ...
            if (isset($_data['sender_acl'])) {
              ...
              if (!empty($sender_acl_domain_admin) || !empty($sender_acl_admin)) {
                ...
                foreach ($sender_acl_domain_admin as $key => $val) {
                  ...
                  if ($val == '*' && $_SESSION['openemail_cc_role'] != 'admin') {
                    $_SESSION['return'][] = array(
                      'type' => 'danger',
                      'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
                      'msg' => array('sender_acl_invalid', $sender_acl_domain_admin[$key])
                  ...
                  if (filter_var($val, FILTER_VALIDATE_EMAIL)) {
                    if (!hasAliasObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $val)) {
                      $_SESSION['return'][] = array(
                        'type' => 'danger',
                        'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
                        'msg' => array('sender_acl_invalid', $sender_acl_domain_admin[$key])
        ...
        case 'resource':
          ...
          foreach ($names as $name) {
            ...
            if (!hasMailboxObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $name)) {
              $_SESSION['return'][] = array(
                'type' => 'danger',
                'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
                'msg' => 'access_denied'
      ...
      switch ($_type) {
        case 'sender_acl_handles':
          ...
          while ($domain_row = array_shift($domain_rows)) {
            if (is_valid_domain_name($domain_row['send_as']) && !hasDomainAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $domain_row['send_as'])) {
            ...
            if ($domain_row['send_as'] == '*' && $_SESSION['openemail_cc_role'] != 'admin') {
              $data['sender_acl_domains']['ro'][] = $domain_row['send_as'];
            }
            if ($domain_row['send_as'] == '*' && $_SESSION['openemail_cc_role'] == 'admin') {
              $data['sender_acl_domains']['rw'][] = $domain_row['send_as'];
          ...
          while ($address_row = array_shift($address_rows)) {
            if (filter_var($address_row['send_as'], FILTER_VALIDATE_EMAIL) && !hasAliasObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $address_row['send_as'])) {
          ...
          while ($row_domain = array_shift($rows_domain)) {
            if (is_valid_domain_name($row_domain['domain']) && hasDomainAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $row_domain['domain'])) {
              $data['sender_acl_domains']['selectable'][] = $row_domain['domain'];
              continue;
            }
            if ($row_domain['domain'] == '*' && $_SESSION['openemail_cc_role'] == 'admin') {
              $data['sender_acl_domains']['selectable'][] = $row_domain['domain'];
              continue;
            }
          }
          ...
          while ($row = array_shift($rows_mbox)) {
            ...
            if (filter_var($row['address'], FILTER_VALIDATE_EMAIL) && hasAliasObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $row['address'])) {
        ...
        case 'mailboxes':
          $mailboxes = array();
          if (isset($_data) && !hasDomainAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $_data)) {
            return false;
          }
          elseif (isset($_data) && hasDomainAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $_data)) {
            ...
            $stmt->execute(array(
              ':username' => $_SESSION['openemail_cc_username'],
              ':role' => $_SESSION['openemail_cc_role'],
            ));
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            while($row = array_shift($rows)) {
              $mailboxes[] = $row['username'];
        ...
        case 'filter_details':
          ...
          if (!hasMailboxObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $filter_details['username'])) {
            return false;
          }
        ...
        case 'syncjob_details':
          ...
          if (!hasMailboxObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $syncjobdetails['user2'])) {
            return false;
          }
        ...
        case 'alias_details':
          ...
          if (!hasDomainAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $aliasdata['domain'])) {
            return false;
          }
          return $aliasdata;
        break;
        case 'alias_domain_details':
          ...
          if (!hasDomainAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $aliasdomaindata['target_domain'])) {
            return false;
          }
        ...
        case 'resource_details':
          ...
          if (!isset($resourcedata['domain']) ||
            (isset($resourcedata['domain']) && !hasDomainAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $resourcedata['domain']))) {
            return false;
          }
          return $resourcedata;
        break;
      ...
      switch ($_type) {
        case 'syncjob':
          ...
          foreach ($ids as $id) {
            ...
            if (!hasMailboxObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $user2)) {
              $_SESSION['return'][] = array(
                'type' => 'danger',
                'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
                'msg' => 'access_denied'
        ...
        case 'filter':
          ...
          foreach ($ids as $id) {
            ...
            if (!hasMailboxObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $usr)) {
              $_SESSION['return'][] = array(
                'type' => 'danger',
                'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
                'msg' => 'access_denied'
        ...
        case 'eas_cache':
          ...
          foreach ($usernames as $username) {
            if (!hasMailboxObjectAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $username)) {
              $_SESSION['return'][] = array(
                'type' => 'danger',
                'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
                'msg' => 'access_denied'
        ...
        case 'alias_domain':
          ...
          foreach ($alias_domains as $alias_domain) {
            ...
            $DomainData = $stmt->fetch(PDO::FETCH_ASSOC);
            if (!hasDomainAccess($_SESSION['openemail_cc_username'], $_SESSION['openemail_cc_role'], $DomainData['target_domain'])) {
              $_SESSION['return'][] = array(
                'type' => 'danger',
                'log' => array(__FUNCTION__, $_action, $_type, $_data_log, $_attr),
                'msg' => 'access_denied'
