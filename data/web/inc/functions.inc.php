  ...
  if (!empty($_data['return'])) {
    ...
    foreach ($_data['return'] as $return) {
      ...
      if (!empty($_SESSION["dual-login"]["username"])) {
        $user = $_SESSION["dual-login"]["username"] . ' => ' . $_SESSION['openemail_cc_username'];
        $role = $_SESSION["dual-login"]["role"] . ' => ' . $_SESSION['openemail_cc_role'];
      }
      elseif (!empty($_SESSION['openemail_cc_username'])) {
        $user = $_SESSION['openemail_cc_username'];
...
function check_login($user, $pass) {
	...
	foreach ($rows as $row) {
      ...
      if (get_tfa($user)['name'] != "none") {
        $_SESSION['pending_openemail_cc_username'] = $user;
        $_SESSION['pending_openemail_cc_role'] = "admin";
	...
	foreach ($rows as $row) {
      ...
      if (get_tfa($user)['name'] != "none") {
        $_SESSION['pending_openemail_cc_username'] = $user;
        $_SESSION['pending_openemail_cc_role'] = "domainadmin";
        $_SESSION['pending_tfa_method'] = get_tfa($user)['name'];
        unset($_SESSION['ldelay']);
        $_SESSION['return'][] =  array(
          'type' => 'info',
	...
	elseif (!isset($_SESSION['openemail_cc_username'])) {
...
function rspamd_ui($action, $data = null) {
	...
	switch ($action) {
      ...
      $docker_return = docker('post', 'rspamd-openemail', 'exec', array('cmd' => 'rspamd', 'task' => 'worker_password', 'raw' => $rspamd_ui_pass), array('Content-Type: application/json'));
